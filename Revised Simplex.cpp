...
int main(int argc, const char * argv[]) {
    ...
    // Read in the number of variables and number of constraints
    ...
    printf("\nEnter the coefficients in the objective function respectively:\n");
    for (size_t col = 0; col < n; ++col) {
    ...
    printf("\nEnter the coefficients of the variables and the constants of all the inequations respectively without any symbols:\n");
    for (size_t row = 0; row < m; ++row) {
        for (size_t col = 0; col <= n; ++col) {
            if (col == n) {
                double bRow;
